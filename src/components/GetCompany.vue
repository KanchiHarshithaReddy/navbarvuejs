  ...
  <div>
    ...
    <div class="inline" >
     ...
     <p class="float-left">
      ...
      <select @change="paginateEntries($event)" >
        <option v-for="page in pageEntries" :key="page" :value="page">
          {{ page }}
        </option>
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(user2, i) in filterdEntries" :key="i">
          <td>{{ i + 1 }}</td>
      ...
      axios
        ...
        .then((response) => {
          ...
          Object.keys(this.userdetails).forEach((result) => {
            ...
            this.filterdEntries=this.datalist;
          });
        });
    },
    onFormSubmit(e) {
      ...
      this.companylist = result.data;
        ...
        paginateEntries(eve) {
       this.page1 = eve.target.value;
   
      if (this.page != 0) {
        this.filterdEntries = this.datalist.slice(0, this.page1);
...
.btn {
  margin-left: 1150px;
  background-color: rgb(124, 29, 112);
  margin-bottom: 15px;
  margin-top: 2 px;
}
.result {
